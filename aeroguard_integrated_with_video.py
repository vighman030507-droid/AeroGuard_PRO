# ============================================
# AeroGuard Pro - Integrated ML Dashboard
# Flask App with ML Model Backend Integration
# ============================================
...
def predict_aqi_ml(pm25, pm10, weather, stability):
    """Predict AQI using the ML model"""
    if not MODEL_LOADED:
        # Fallback to simple calculation
        return calculate_aqi_fallback(pm25, pm10, weather, stability)
    
    try:
        # Create feature vector matching the 8 features used in training
        # ['PM2.5_mean', 'PM10_mean', 'weather_mean', 'weather_std',
        #  'AQI_rolling_mean_3', 'AQI_rolling_std_7', 'AQI_lag1', 'AQI_lag3']
        
        # Estimate rolling and lag features based on current values
        aqi_estimate = (pm25 * 2.5 + pm10 * 1.0) * (1 + (weather - 50) / 100)
        
        features = np.array([[
            pm25,                    # PM2.5_mean
            pm10,                    # PM10_mean
            weather,                 # weather_mean
            stability,               # weather_std (using stability as proxy)
            aqi_estimate,            # AQI_rolling_mean_3 (using estimate)
            stability * 0.5,         # AQI_rolling_std_7 (using stability as proxy)
            aqi_estimate * 0.95,     # AQI_lag1 (slight decrease from current)
            aqi_estimate * 0.90      # AQI_lag3 (more decrease)
        ]])
        
        # Scale and predict
        features_scaled = scaler.transform(features)
        aqi_pred = model.predict(features_scaled)[0]
        
        # Ensure valid range
        return min(max(int(aqi_pred), 0), 500)
    
    except Exception as e:
        print(f"ML prediction error: {e}, using fallback")
        return calculate_aqi_fallback(pm25, pm10, weather, stability)

def calculate_aqi_fallback(pm25, pm10, weather, stability):
    """Fallback AQI calculation when ML model is not available"""
    pm25_contribution = pm25 * 2.5
    pm10_contribution = pm10 * 1.0
    weather_factor = 1 + (weather - 50) / 100
    volatility_factor = 1 + stability / 200
...
def generate_forecast_data(current_aqi):
    """Generate 6-hour forecast data using ML model"""
    data = []
    now = datetime.now()
...
<body class="font-sans text-white">
    ...
    <section id="hero" class="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div class="absolute inset-0 overflow-hidden">
            <video 
                ...
                <source src="/static/background-video.webm" type="video/webm">
                <!-- Fallback to animated particles if video doesn't load -->
            </video>
            <!-- Fallback animated particles (shown if video fails to load) -->
            <div id="particleFallback" class="hidden">
        ...
        <!-- Gradient Overlay -->
        <div class="absolute inset-0 bg-gradient-to-b from-background/70 via-background/60 to-background"></div>
    ...
    {% if model_loaded %}
    <section class="py-4 px-4 max-w-7xl mx-auto">
        <div class="card-glass rounded-2xl p-6 text-center">
            <div class="flex flex-wrap items-center justify-center gap-6">
                <div class="flex items-center gap-2">
                    <i data-lucide="cpu" class="w-5 h-5 text-primary"></i>
                    <span class="text-sm">Ensemble Model (RF + GB)</span>
                </div>
                <div class="flex items-center gap-2">
                    <i data-lucide="bar-chart-3" class="w-5 h-5 text-accent"></i>
                    <span class="text-sm">R² Score: {{ '%.3f' % cv_metrics.cv_r2 }}</span>
                </div>
                <div class="flex items-center gap-2">
                    <i data-lucide="layers" class="w-5 h-5 text-aqi-good"></i>
                    <span class="text-sm">8 Advanced Features</span>
                </div>
                <div class="flex items-center gap-2">
                    <i data-lucide="target" class="w-5 h-5 text-aqi-moderate"></i>
                    <span class="text-sm">MAE: {{ '%.1f' % metrics[0] }}</span>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="space-y-6">
                ...
                <div id="risk" class="card-glass rounded-2xl p-6 transition-all">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        {% if model_loaded %}
                        <span class="ml-auto ml-badge text-xs">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fallback = document.getElementById('particleFallback');
            
            // Show fallback if video fails to load
            video.addEventListener('error', function() {
                console.log('Video failed to load, using particle fallback');
            ...
            video.play().catch(function(error) {
                ...
                // Video will start when user interacts with page
            });
        ...
        function generateInsights(pm25, pm10, weather, stability, aqi) {
            ...
            if (modelLoaded) {
                insights.push({ icon: 'zap', text: 'Predictions powered by advanced ensemble ML model (Random Forest + Gradient Boosting) trained on 211K+ data points.' });
            }
        ...
        // Update Dashboard - Now calls backend API
        async function updateDashboard() {
            ...
            try {
                // Call backend API for ML prediction
                ...
                // Update Forecast Chart with backend data
                if (forecastChart && forecast) {
                    const forecastLabels = forecast.map(f => f.hour);
                    const forecastData = forecast.map(f => f.aqi);
...
def index():
    return render_template_string(
        ...
        metrics=metrics if MODEL_LOADED else (0, 0),
        cv_metrics=cv_metrics if MODEL_LOADED else {'cv_r2': 0, 'cv_std': 0}
    )

@app.route('/api/predict', methods=['POST'])
def api_predict():
    """API endpoint for ML-powered AQI prediction"""
    data = request.json
    pm25 = data.get('pm25', 45)
    pm10 = data.get('pm10', 80)
    weather = data.get('weather', 65)
    stability = data.get('stability', 30)
    city = data.get('city', 'Delhi')
    
    # Use ML model for prediction
    aqi = predict_aqi_ml(pm25, pm10, weather, stability)
    ...
    return jsonify({
        ...
        'forecast': forecast,
        'model_used': 'ensemble_ml' if MODEL_LOADED else 'fallback',
        'city': city
    })
...
def api_model_info():
    ...
    if MODEL_LOADED:
        ...
        return jsonify({
            'loaded': False,
            'message': 'ML model not loaded. Using fallback calculations.'
        })
...
if __name__ == '__main__':
    ...
    print("   📹 Place your video file at: ./static/background-video.mp4")
    print("="*60 + "\n")
